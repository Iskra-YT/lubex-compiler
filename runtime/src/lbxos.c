...
void __OS_exit(int exitCode) {
    asm volatile("syscall" : : "a"(OS_EXIT), "D"(exitCode) : "rcx", "r11", "memory");
}
...
long __OS_output(const char* buffer, long length) {
    ...
    asm volatile("syscall" : "=a"(ret) : "a"(OS_WRITE), "D"(1), "S"(buffer), "d"(length) : "rcx", "r11", "memory");
...
long __OS_input(char* buffer, long length) {
    ...
    asm volatile("syscall" : "=a"(ret) : "a"(OS_READ), "D"(0), "S"(buffer), "d"(length) : "rcx", "r11", "memory");
