...
void Parser::initVarDecl() {
         ...
         [&](Token& t, void* ctx) {
             ...
             if (getCurrent().match(Token("?", TokenType::DELIMITER_TOKEN))) {
                 ...
                 c.type = std::make_unique<NullableTypeNode>(
                     PositionSpan(c.type->position.start, getCurrent().position.end), std::move(c.type));
             }
         },
         false, 2},
    ...
    varDeclInstr.finalize = [](PositionSpan span, void* ctx) {
        ...
        if (c->valueNode) {
            return std::make_unique<VariableDeclarationNode>(span, std::move(c->name), std::move(c->valueNode),
                                                             std::move(c->type), false);
        } else {
            return std::make_unique<VariableDeclarationNode>(span, std::move(c->name), nullptr, std::move(c->type),
                                                             false);
        }
    };
}
...
void Parser::initFuncDecl() {
    funcDeclInstr.steps = {{TokenType::KEYWORD_TOKEN, "func", [](Token&, void*) {}, true, 0},
                           {TokenType::IDENTYFIER_TOKEN, "",
                            [&](Token&, void* ctx) {
    ...
    funcDeclInstr.finalize = [](PositionSpan span, void* ctx) {
        ...
        return std::make_unique<FunctionDeclaration>(span, std::move(c->name), std::move(c->returnType),
                                                     std::move(c->parameters), std::move(c->body), c->isForward,
                                                     c->isStatic, c->visibility, c->isOverride);
    };
}

void Parser::initClassDecl() {
    classDeclInstr.steps = {{TokenType::KEYWORD_TOKEN, "class", [](Token&, void*) {}, true, 0},
                            {TokenType::IDENTYFIER_TOKEN, "",
                             [&](Token& t, void* ctx) {
                                 allowCallAndMember = false;
    ...
    classDeclInstr.finalize = [](PositionSpan span, void* ctx) {
        ...
        return std::make_unique<ClassDeclNode>(span, std::move(c->name), std::move(c->members), c->isForward,
                                               c->visibility, std::move(c->extender));
    };
}
...
void Parser::initReturnDecl() {
    returnDeclInstr.steps = {{TokenType::KEYWORD_TOKEN, "return", [](Token&, void*) {}, true, 0},
                             {TokenType::ANY, "",
                              [&](Token& t, void* ctx) {
                                  auto& c = *(ReturnDeclContext*)ctx;
                                  if (!t.match(Token(";", TokenType::DELIMITER_TOKEN)) &&
...
void Parser::initIncludeDecl() {
    includeInstr.steps = {{TokenType::KEYWORD_TOKEN, "import", [](Token&, void*) {}, true, 0},
                          {TokenType::IDENTYFIER_TOKEN, "",
                           [&](Token& t, void* ctx) {
